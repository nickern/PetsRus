...
public class RemovePattedyr extends JDialog {

	/**
	 * 
	 */
	private static final long serialVersionUID = 2533783025109031137L;
	private final JPanel contentPanel = new JPanel();
	private JPanel buttonPane;
	JList<Pattedyr> listSolgt;
	...
	public RemovePattedyr() {
				...
				okButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
					 	...
					 	try{
							...
							MainFrame.getPattedyrList().remove(listSolgt.getSelectedIndex());
		...
		int pattedyrN = MainFrame.getPattedyrList().size();
		Pattedyr[] patArray = new Pattedyr[pattedyrN];
		
		// lopierer objectreferanser fra lister til arrays
		MainFrame.getPattedyrList().toArray(patArray);
		
		// populer combobox
				// Remove comment to use the designer.
	//			JList<Pattedyr> listSolgt = new JList();
				// Comment this out to use the designer.
		listSolgt = new JList<Pattedyr>(patArray);
