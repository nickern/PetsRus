...
public class RemoveFisk extends JDialog {

	/**
	 * 
	 */
	private static final long serialVersionUID = 2126927765469395206L;
	private final JPanel contentPanel = new JPanel();
	private JPanel buttonPane;
	JList<Fisk> listSolgt;
	...
	public RemoveFisk() {
				...
				okButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
					 	...
					 	try{
							...
							MainFrame.getFiskList().remove(listSolgt.getSelectedIndex());
		...
		int fiskN = MainFrame.getFiskList().size();
		Fisk[] fiskArray = new Fisk[fiskN];
		
		// lopierer objectreferanser fra lister til arrays
		MainFrame.getFiskList().toArray(fiskArray);
		
		// populer combobox
				// Remove comment to use the designer.
	//			JList<Fisk> listSolgt = new JList();
				// Comment this out to use the designer.
		listSolgt = new JList<Fisk>(fiskArray);
